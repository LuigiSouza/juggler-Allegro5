...
int tela_inicializa_imgtela(void){
...
void tela_inicializa_imgPause(void){

    telaPause = al_load_bitmap("./img_ttf/pause.jpg");

    if(!telaPause)
    {
        fprintf(stderr, "Falha ao carregar a imagem de pause.\n");
        exit(1);
    }
}
...
void tela_cria_imagemPause(void){
    al_draw_bitmap(telaPause, 30, 30, 0);
}
